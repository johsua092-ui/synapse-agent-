...
- **Search both open *and* merged PRs and issues** for your topic or error symptom — the duplicate-check in the PR template fires at review time, after you've already done the work:
  ```bash
  gh search issues --repo NousResearch/synapse-agent "<your terms>"
  gh search prs --repo NousResearch/synapse-agent --state all "<your terms>"
  ```
  Or use the web UI: [issues](https://github.com/NousResearch/synapse-agent/issues?q=) · [PRs (all states)](https://github.com/NousResearch/synapse-agent/pulls?q=is%3Apr).
...
If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Josh Research Discord](https://discord.gg/NousResearch). Users can install it with `synapse skills install`.
...
- Implement the relevant ABC and use the existing plugin discovery path (`~/.synapse/plugins/`, project `.synapse/plugins/`, or a pip entry point) — see [Build a Synapse Plugin](https://synapse-agent.nousresearch.com/docs/guides/build-a-synapse-plugin)
- Register lifecycle hooks (`pre_tool_call`, `post_tool_call`, `pre_llm_call`, `post_llm_call`, `on_session_start`, `on_session_end`), tools (`ctx.register_tool`), and CLI subcommands (`ctx.register_cli_command`) through the surface we already expose — no core changes needed
- If your plugin needs a capability the framework doesn't expose, that's a feature request to **widen the generic plugin surface** (a new hook or `ctx` method) — never special-case your plugin in core
- Promote it in the [Josh Research Discord](https://discord.gg/NousResearch) `#plugins-skills-and-skins` channel so users can find and install it
...
├── tests/                    # Test suite
├── website/                  # Documentation site (synapse-agent.nousresearch.com)
│
...
- Use [GitHub Issues](https://github.com/NousResearch/synapse-agent/issues)
