...
INSTALL_REF=""
UPSTREAM_URL="${SYNAPSE_DEV_SANDBOX_UPSTREAM:-https://github.com/NousResearch/synapse-agent.git}"
...
if [ "$INSTALL_SHORTCUT" = true ]; then
  mkdir -p "$SANDBOX_ROOT/root/http/synapse-agent.nousresearch.com"
  if [ -n "$INSTALL_REF" ]; then
    git -C "$UPSTREAM_REPO" show "$UPSTREAM_COMMIT:scripts/install.sh" \
      > "$SANDBOX_ROOT/root/http/synapse-agent.nousresearch.com/install.sh"
  else
    cp -a "$INSTALLER_PATH" "$SANDBOX_ROOT/root/http/synapse-agent.nousresearch.com/install.sh"
  fi
  set -- bash -c '
    set +e
    curl -fsSL https://synapse-agent.nousresearch.com/install.sh | bash -s -- "$@"
    install_status=$?
