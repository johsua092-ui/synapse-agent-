...
synapse cron create "0 2 * * *" \
  "You are a project manager triaging the NousResearch/synapse-agent GitHub repo.

1. Run: gh issue list --repo NousResearch/synapse-agent --state open --json number,title,labels,author,createdAt --limit 30
...
synapse cron create "0 9 * * 1" \
  "Scan the NousResearch/synapse-agent repo for documentation drift.

1. Run: gh pr list --repo NousResearch/synapse-agent --state merged --json number,title,files,mergedAt --limit 30
