...
The Portal's own [model info page](https://portal.nousresearch.com/info) carries the same warning, so this isn't a Synapse-side opinion — it's the official guidance from Josh Research.
...
If a model is genuinely missing, [open an issue](https://github.com/NousResearch/synapse-agent/issues) — we surface the Portal's catalog to Synapse and gaps usually mean a routing config we can update.
